...
def perform_ocr_on_pdf(pdf_path):
    ...
    try:
        ...
        print("\nOCR処理を開始...")
        print("⚠ 初回実行時はモデルのダウンロードで時間がかかります")

        # EasyOCR Readerを初期化（日本語と英語）
        print("OCRエンジンを初期化中（日本語+英語）...")
        reader = easyocr.Reader(['ja', 'en'], gpu=False)

        # PDFを開く
        doc = fitz.open(pdf_path)
        print(f"PDF読み込み完了: {len(doc)} ページ")

        # 各ページにOCRを実行してテキストレイヤーを追加
        for page_num in range(len(doc)):
            print(f"ページ {page_num + 1}/{len(doc)} を処理中...")
        ...
        print("  インストール方法:")
        print("  pip install easyocr pymupdf")
        return None
    except Exception as e:
